function varargout = pairs5(SPX218, selectedPairs, M, N, spreadH, spreadL, scaling, cost, trainSet, capital)
...
%else
    ...
    for i = 1:nargout
        switch i
            case 1
                varargout{1} = var95; % signal
            case 2
                varargout{2} = var99; % return (pnl)
            case 3
                varargout{3} = cVar95; % sharpe ratio
            case 4
                varargout{4} = cVar99; % indicator
            case 5
                varargout{5} = maxDD; % indicator
            case 6
                varargout{6} = sortinoRatio; % indicator
            case 7
                varargout{7} = skewnessRet;
            case 8
                varargout{8} = kurtosisRet;
            case 9
                varargout{9} = beta;
            case 10
                varargout{10} = adjustedAlpha;
            case 11
                varargout{11} = percentReturn;
            otherwise
